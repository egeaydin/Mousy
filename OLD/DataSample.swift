...
class DataSample
{
    ...
    var mechanicalFilterLimit: Double = 0
    ...
    private(set) var sumOfData: Double
    ...
    func operate(_ value: Double)
    {
        ...
        let calibrated = calibrate(value: filter(value:value))
        let validated = validateData(calibrated)
        _filteredData.trail(calibrated)
        filteredData.append(ChartDataEntry(x:time, y:validated))
    ...
    /**
        ...
        - Parameters:
            - value: The value about to be filtered by the kalman filter
    ...
    */
    ...
    /**
     The purpose of the calibration routine is to obtain the value of the reference threshold. It eliminaties zero reference offset of the acceleration data, It subtacts the current avarage from the value so there will be negative and positive acceleration.
     - Parameters:
        - value: Value that is getting calibrated
     - Returns: the calibrated value
     - Complexity: O(1)
     */
    private func calibrate(value: Double) -> Double
    {
        let avarage = sumOfData/Double(data.count)
